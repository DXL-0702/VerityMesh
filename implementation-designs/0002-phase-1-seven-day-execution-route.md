...
| 在线 AI | 项目范围校验、查询计划、混合检索、RRF、Reranker、Evidence、生成、Grounding、Citation 和跨项目路由 | 不修改 PostgreSQL 权威业务状态，不接受客户端自造范围 |
...
| 工具版本 | 前端包管理器、Node、Java 构建工具、JDK、Python、uv 和 Schema 生成方式已经固定 | 先完成工具选型和版本冻结 |
| 本地集成环境 | PostgreSQL、Kafka、Redis Online、Redis Celery、Elasticsearch 和对象存储兼容环境可以访问 | 先搭建环境 |
| 真实云联调环境 | PostgreSQL、Kafka、Redis、Elasticsearch、网关、身份、安全、解析和可观测性产品已有批准的联调环境 | 只能继续本地开发，不能关闭生产门禁 |
...
| 层次 | 要做的事情 | 当天必须交付 |
| --- | --- | --- |
| 跨层接口与集成 | 定义 Public OpenAPI、SSE 事件、Java 到 Python 执行上下文、Kafka 事件、Celery JSON 任务、统一错误和幂等规则 | 第一版接口文件、示例和兼容测试 |
| 前端 | 创建 Portal、Assistant UI 和 TypeScript Client 工程；接入生成的 API 类型；实现项目列表、知识空间、上传和任务状态页面 | 可以选择项目并上传文件的页面 |
| Java 后端 | 创建模块化 platform-api；实现 Project、Project Version、Knowledge Space、Source Object、Source Revision、Task 和 Outbox | 数据库迁移、上传 API、SourceRevision 和 Outbox 事务 |
| 在线 AI | 创建 uv Workspace 和 FastAPI 内部入口；实现执行上下文格式校验、Deadline 和 Provider 测试替身 | 只接受内部已认证请求的 Runtime 入口 |
| 知识批处理 | 创建 Kafka Dispatcher 和 Celery Worker；实现 JSON 任务、幂等键和 Source Revision 消费 | 可以接收 SourceRevision 事件并创建任务 |
| 基础设施与测试 | 创建本地集成环境、CI 和统一验证脚本；建立 Trace ID 传播检查 | 干净 Checkout 可构建并运行合同测试 |
...
用户在治理页面上传文件
  -> 文件进入隔离对象存储
  -> Java 在 PostgreSQL 登记 SourceRevision、Task 和 Outbox
  -> Outbox 发布 Kafka 事件
...
打通“Source Revision -> 安全扫描 -> 解析/OCR -> 去重 -> Chunk -> Embedding -> Staging Index -> 发布评测”。
...
| Java 后端 | 实现治理状态、编辑/审批权限、Worker 结果投影、Knowledge Revision 和 Release Candidate | PostgreSQL 中可追踪的治理与候选状态 |
| 在线 AI | 实现 EmbeddingPort、模型 Revision 校验、批量限制、错误分类和出站最小化 | 可替换且可审计的 Embedding Adapter |
| 知识批处理 | 实现扫描、解析/OCR、SHA-256 去重、MinHash/LSH 审核信号、结构感知 Chunk、Embedding、Staging 写入和增量评测 | 从源文件生成待发布候选 |
| 基础设施与测试 | 测试恶意文件、Secret/PII、超限文件、解析失败、模型超时、重复事件和批量写部分失败 | 批处理失败与恢复报告 |
...
3. 使用的 Chunker、Embedding 模型 Revision 和索引版本。
4. 检索、ACL 和泄露评测结果。
5. 候选是否具备提交审批的资格。
...
- Release Candidate 固定 Analyzer、Chunker、Embedding Space、模型 Revision、RRF、Reranker 和 Index Schema Version。
...
| 在线 AI | 实现范围/撤回校验、ProjectQueryPlan、BM25 和 Vector 独立召回、RRF、Reranker、Evidence Hub、Prompt、Generator、Grounding、Citation 和已验证 SSE | 固定项目 RAG 全链路 |
| 知识批处理 | 实现发布评测、ActivationRequested、Published Alias 原子切换、Tombstone、投影清理和索引重建 | 可激活、可回滚、可撤回的 Published Projection |
...
| Java 后端 | 测试事务回滚、Outbox 原子性、并发审批、Token 撤销、Thread 乐观锁、审计完整性和优雅终止 | Java 可靠性与安全报告 |
| 在线 AI | 测试 Provider 超时、限流、无效输出、BM25-only、RRF fallback、Evidence-only、拒答、模型回退和 Prompt Injection | AI 降级与安全报告 |
| 知识批处理 | 测试 Kafka 重放、Celery 重试/崩溃/DLQ、全量与增量隔离、Alias 失败、Tombstone 和 Elasticsearch 重建 | 批处理恢复报告 |
| 基础设施与测试 | 执行性能、容量、成本、安全、故障注入、备份恢复、Trace/Metric/Log 关联和 Runbook 演练 | 生产前预验收报告 |
...
| 2 | 运行单元、合同、组件、跨服务和数据库迁移测试 | 完整自动化测试报告 |
...
| Java 后端 | platform-api、数据库迁移、身份授权、Session/Thread、Knowledge/Release、Outbox、审计和测试 |
| 在线 AI | FastAPI Runtime、受约束 RAG Kernel、Model Adapter、Project/Global 路由、Evidence/Citation/Grounding 和质量测试 |
| 知识批处理 | Kafka Dispatcher、Celery Worker、解析/OCR、Chunk、Embedding、索引、评测、激活、撤回和恢复测试 |
| 数据与部署 | PostgreSQL、OSS、Kafka、双 Redis、Elasticsearch 配置，以及四个 Deployment 的网络、身份、扩缩容和备份定义 |
