...
- 不实现第二阶段的订单、账号、权益、工单等业务 API Connector、ToolPlan 或写操作。
- 不实现第三阶段 GraphRAG、图数据库或跨项目混合图。
- 不引入 LangChain 主 RAG、LangGraph 主 Workflow 或 MaxKB Runtime。
- 不建设 React Adapter、原生移动 SDK、Flink、独立向量数据库、私有模型生产链路或多地域灾备。
...
| 在线 AI 层 | `assistant-runtime` | Python + FastAPI + `uv` | 不可变 Execution Context Guard、Project/Global 查询计划、混合检索、Evidence、Prompt、模型访问、Grounding、Citation 和已验证事件 | 不直接修改 PostgreSQL 权威状态，不接受客户端自造 Scope，不流出未验证模型 Token |
| 知识批处理层 | `batch-worker` | Python + Celery + `uv` | Kafka 事件分发、扫描、解析、OCR、去重、Chunk、Embedding、Staging Index、发布评测、激活和投影清理 | 不成为业务状态机，不决定审批和 Release 真相，不接受 Java 直接写入 Celery 私有协议 |
| 权威数据层 | PostgreSQL、OSS、Kafka | 关系状态、不可变内容、可重放事件 | 保存业务状态、Source/Governance/Revision 内容和领域事件 | 不允许 Redis 或 Elasticsearch 成为 Release、授权或任务状态真相 |
| 在线投影层 | Elasticsearch、Redis Online、Redis Celery | Published/Staging 检索投影与短期状态 | 提供受约束检索、在线 Session/撤回缓存和 Celery Broker/短期 Result | 不长期共享在线与 Celery 的容量、ACL、淘汰或故障边界 |
...
| Evaluation Report Schema | `batch-worker` | `platform-api`、治理 UI | Retrieval、Answer、ACL、Leakage、容量和成本指标；数据集版本、模型版本、索引版本和通过状态 |
...
- 所有 Source 接入先将对象写入隔离 OSS Source Zone，再在 PostgreSQL 事务中登记 SourceRevision、Task State 和 Outbox Record。
...
- 实现 Emergency Revoke 同步安全路径：在 PostgreSQL 提交权威撤回状态后更新 Redis Online Revocation List，再异步发布 Tombstone 清理投影。
...
- PostgreSQL 集成测试覆盖事务回滚、Outbox 原子性、重复事件、乱序事件、投影幂等和并发审批。
...
- 实现 Elasticsearch BM25 与 Vector 独立召回，Project、Release、Locale、Access Segment、有效期、撤回状态、Embedding Space 和配置过滤在各路 Top-K 前生效。
...
- Embedding Space、Tokenizer/Model Revision、Analyzer、Chunker、RRF、Reranker 和 Index Schema Version 写入 Release Manifest。
- Staging 与 Published Projection 隔离；只有 Java 已批准的 ActivationRequested 才能执行 Alias/Deployment Revision 激活。
- Tombstone 清理 BM25、Vector、缓存和 Published Projection；完成后发布可重放结果事件。
...
- 测试回滚、紧急撤回、删除传播和 Elasticsearch 从 Release Manifest、OSS、Kafka 重建。
...
| 组件 | 第一阶段建设内容 | 故障与恢复原则 |
| --- | --- | --- |
| PostgreSQL | 业务、身份映射、Grant、Session、Release、任务状态、Conversation、Outbox 和审计 Schema；迁移、备份和 PITR | 权威状态；初始 `RPO <= 5 分钟`、`RTO <= 2 小时` |
| OSS | Source、Governance、Immutable Revision 和发布资产的 Bucket/Prefix、版本控制、加密和生命周期 | 内容权威；对象路径不进入公开响应 |
...
| Elasticsearch | Staging/Published Index Template、Analyzer、向量字段、过滤字段、Alias 和 Watermark | 可从 Release Manifest、OSS 和 Kafka 重建 |
...
knowledge_release_id / index_watermark
...
| --- | --- | --- | --- | --- | --- | --- |
| `P1-00` | 合同与测试基座 | 消费 OpenAPI/SSE 夹具，建立共享状态机测试 | 固定 Public API、内部 Context、事件和错误合同 | 固定领域 DTO、Retrieval/Evidence/Model Port 合同 | 固定 Kafka/Celery/Evaluation Schema；准备本地可替代依赖和密封测试集 | Schema 兼容测试通过；所有层能用同一 Trace/ID/错误语义集成 |
| `P1-01` | Project 与 Source 接入 | Project、Version、Space、上传和任务页 | Project Catalog、SourceRevision、OSS 登记、Outbox | 仅建立受认证内部入口与 Context 校验 | Source Zone、Kafka Dispatcher、Celery 幂等任务 | 上传只能进入 Source Zone；事务、Outbox 和重复消费通过 |
| `P1-02` | 治理与 Staging 构建 | 治理审核、扫描/解析结果和失败处置 | 治理状态、审核权限、Worker 结果投影 | 提供 Embedding/模型任务端口合同 | Scan、Parse/OCR、Deduplicate、Chunk、Embed、Staging Index | 原文 Offset/Citation 可重建；恶意或未审核内容不能进入 Staging Candidate |
| `P1-03` | 单项目检索与回答 | Project Search/Chat、Citation、拒答和降级 | PUBLIC 短期 Session、Message、Search、Context、SSE 代理 | Project Plan、BM25/Vector、RRF、Reranker、Evidence、Generator、Grounding | 检索和回答评测集、Published 测试投影 | 质量、过滤、Citation、未验证输出和延迟门禁达到 1A 基线 |
| `P1-04` | 发布、回滚、撤回和删除 | Candidate 报告、双人审批、换版、回滚、撤回状态 | Release 状态机、Activation、Rollback、Revoke、Successor Session | 每请求复核 Release/Revocation；旧租约拒绝 | 评测、Alias 激活、Tombstone、投影重建 | “导入到删除传播”全链路通过；紧急撤回 P95 不超过 60 秒 |
...
| `P1-11` | 第一阶段生产门禁 | 全入口回归、可访问性和用户流程 | 全状态机、安全、审计、备份恢复 | 全质量、模型回退和延迟矩阵 | 容量、成本、可用性、恢复和删除演练 | `tech-plan.md` 第 13 章全部门禁关闭并形成可审计报告 |
...
| 组件集成测试 | PostgreSQL 事务/Outbox、Redis 原子操作、Kafka 重放、Celery 重试、Elasticsearch Filter/Rank/Alias 和模型 Adapter |
...
- 固定 Provider、模型、Tokenizer、Embedding Space、Analyzer、Chunker、Prompt 和 Index Schema Revision 的评测 Manifest。
...
| `P1-01` 权威数据与事件 | 托管 PostgreSQL 产品/版本、Conversation 隔离、Kafka 兼容版本与托管产品、Redis 兼容版本与托管产品、Online/Celery 隔离参数 | `DATA-002`、`DATA-003`、`DATA-004`、`DATA-005`、`DATA-007`、`DATA-008`、`DATA-010`、`DATA-011`、`DATA-012` |
...
| `P1-03` 检索与回答 | Elasticsearch 主候选/挑战者真实实例、IK、Provider 合同、Generator、Grounding、Embedding Revision 和 Reranker Revision | `RET-001`、`RET-002`、`RET-003`、`RET-010`、`MODEL-001`、`MODEL-002`、`MODEL-004`、`MODEL-005`、`MODEL-006`、`MODEL-007`、`MODEL-008`、`MODEL-009`、`MODEL-010`、`MODEL-011`、`MODEL-012`、`MODEL-013`、`MODEL-014`、`MODEL-015`、`MODEL-026`、`MODEL-027` |
...
- 数据库变更使用向前兼容迁移和显式回滚/补偿方案；跨版本滚动部署时才允许有删除条件的短期兼容合同。
- Knowledge Release 使用 Staging 评测、人工审批、Canary 和 Alias/Deployment Revision 原子激活；失败继续服务旧 Release。
- 服务发布必须验证 SSE 排空、幂等重试、Worker Checkpoint、Kafka 重放、Redis 丢失、模型回退和 Elasticsearch 重建。
