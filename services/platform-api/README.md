...
| 当前状态 | P1-00/P1-01 最小知识接入基座已实现；真实 OSS、MySQL、Kafka 和 Outbox Publisher 仍待集成 |
...
Flyway Migration 归本服务所有，但应用进程默认禁用 Flyway，只允许独立预部署 Migration Job 执行。当前已建立 Project/SourceObject/SourceRevision/ProcessingTask/Outbox 的 MySQL V1 基线、上传预约/完成入口和幂等校验；`UnavailableSourceStorage` 明确阻止在未接入 OSS 时伪造上传成功。真实 MySQL、OSS、Kafka Outbox Publisher、身份授权和业务模块仍待后续批次实现。完整技术栈见 [`技术栈与外部选型总览`](../../docs/technology-selection/technology-selection.md)，职责边界见 [`第一阶段执行方案`](../../docs/implementation-designs/0001-phase-1-execution-plan.md)。
