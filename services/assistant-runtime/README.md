...
| 包管理 | `uv` |
| 当前状态 | 在线入口与 Execution Context 安全基线已建立；业务 RAG 尚未实现 |
...
当前仍不包含正式跨语言 Schema、ProjectQueryPlan、Revocation Guard、检索、Provider Adapter、RAG Kernel 或 SSE。版本总览见 [`技术栈与外部选型总览`](../../docs/technology-selection/technology-selection.md)，后续交付边界见 [`第一阶段执行方案`](../../docs/implementation-designs/0001-phase-1-execution-plan.md)。本地可执行版本以成员 `pyproject.toml` 和根 `uv.lock` 为准。
