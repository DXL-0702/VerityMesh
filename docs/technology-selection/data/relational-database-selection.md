...
| Java Schema Migration | 版本化管理 MySQL DDL、索引、约束和迁移校验 | Flyway | 精确版本随 Java 依赖锁定 | `SELECTED` | `platform-api` 独立 Migration Job | 1A | Liquibase 仅在 Flyway 出现明确能力缺口时重评 | Spring Boot 集成直接、SQL-first、迁移历史和校验语义清晰 | 冻结版本并验证空库、N-1 升级、中断重试和滚动兼容 |
| Python Schema Migration | 版本化管理 PostgreSQL `vector` 扩展、Projection 表和向量索引 | Alembic | 精确版本由 `uv.lock` 锁定 | `SELECTED` | Python 独立 Migration Job | 1A | 无第一阶段替代 | 与 Python/SQLAlchemy 生态一致，支持显式 Migration Revision 和离线 SQL 审核 | 冻结版本并验证单一 Head、空库、N-1 升级、中断重试和滚动兼容 |
