...
| 项目业务 API Connector / Tool Executor | 经用户授权实时读取订单、余额、工单等业务数据 | 暂不引入 | 受控 Tool Adapter | `DEFERRED` | Assistant Tool Runtime | 第二阶段 | 只返回知识库 Evidence，不调用实时业务 API | 属于业务工具生态，不应混入第一阶段通用知识索引 | 第二阶段另行评审 Tool Contract 与 OAuth 边界 |
