...
状态绑定到精确 Decision ID，不能从上级类别推导。例如 `PostgreSQL = SELECTED` 不代表 `RDS PostgreSQL = SELECTED`。
...
| `DATA-001` | 数据 | 关系数据库技术 | Technology | `SELECTED` | PostgreSQL | [关系数据库](data/relational-database-selection.md) | 冻结主版本和扩展清单 | 2026-08-05 |
| `DATA-002` | 数据 | 托管 PostgreSQL 形态 | Product Class | `CONFIRMED_WITH_GATES` | 阿里云托管 PostgreSQL 兼容产品 | [关系数据库](data/relational-database-selection.md) | RDS 与 PolarDB 同条件 PoC | 2026-08-05 |
| `DATA-003` | 数据 | RDS PostgreSQL 候选 | Product | `UNSELECTED` | 阿里云 RDS PostgreSQL | [关系数据库](data/relational-database-selection.md) | 与 PolarDB 同条件 PoC | 2026-08-05 |
| `DATA-004` | 数据 | PolarDB PostgreSQL 候选 | Product | `UNSELECTED` | 阿里云 PolarDB PostgreSQL | [关系数据库](data/relational-database-selection.md) | 与 RDS 同条件 PoC | 2026-08-05 |
| `DATA-005` | 数据 | Conversation 物理隔离 | Topology | `UNSELECTED` | 共享库、独立命名空间/库或独立实例待选 | [关系数据库](data/relational-database-selection.md) | 结合容量、保留和故障域决定 | 2026-08-05 |
| `DATA-006` | 数据 | ACK 自建 PostgreSQL | Product Form | `REJECTED` | 第一阶段不采用 | [关系数据库](data/relational-database-selection.md) | 所有托管候选存在硬缺口时重新立项 | 2026-08-05 |
...
| `RET-001` | 检索 | 首期混合搜索引擎 | Product Class | `CONFIRMED_WITH_GATES` | 阿里云 Elasticsearch 向量增强版 8.17 | [搜索引擎](retrieval/search-engine-selection.md) | 真实企业语料与隔离云实例关闭质量、权限、容量、成本和退出门禁 | 2026-08-10 |
| `RET-002` | 检索 | 阿里云 Elasticsearch 主候选 | Product | `CONFIRMED_WITH_GATES` | 阿里云 Elasticsearch 向量增强版 8.17 | [搜索引擎](retrieval/search-engine-selection.md) | 完成真实实例联合矩阵及撤回、删除、回滚合同验证 | 2026-08-10 |
| `RET-003` | 检索 | 阿里云 OpenSearch 挑战者 | Product | `CHALLENGER_POC` | 阿里云 OpenSearch 向量检索版 | [搜索引擎](retrieval/search-engine-selection.md) | 与 Elasticsearch 使用同一语料和硬门禁完成真实实例 PoC | 2026-08-10 |
...
| `RET-005` | 检索 | 独立向量数据库 | Product | `DEFERRED` | Milvus | [向量数据库](retrieval/vector-database-selection.md) | 首期向量能力不达标时启动 PoC | 2026-08-05 |
| `RET-006` | 检索 | 一体化替代引擎 | Product | `DEFERRED` | Weaviate | [向量数据库](retrieval/vector-database-selection.md) | 证明能整体替代并降低复杂度后重评 | 2026-08-05 |
| `RET-007` | 检索 | 三套重叠检索系统长期并存 | Topology | `REJECTED` | 不采用 Elasticsearch + Milvus + Weaviate 长期并存 | [向量数据库](retrieval/vector-database-selection.md) | 仅检索架构正式重构时重评 | 2026-08-10 |
...
| `IDSEC-008` | 身份安全 | 审计防篡改归档 | Product | `UNSELECTED` | PostgreSQL 审计的长期受控归档 | [审计归档](identity-security/audit-archive-selection.md) | 冻结范围、保留和不可篡改要求 | 2026-08-05 |
