...
- Java/Spring Boot `platform-api` 保存业务、身份、授权、Session、Release 和审计真相；Python 只承担在线 AI 与离线知识处理，不直接拥有权威业务状态。
- PostgreSQL 保存权威元数据，阿里云 Elasticsearch 保存可重建的 BM25/向量检索投影，对象存储保存原始和治理内容。
...
- 用户会话或上传内容自动转为正式知识。
- iOS/Android 原生 SDK。
- GraphRAG、独立向量数据库、Flink、私有模型和多地域灾备的生产依赖。
...
个性化订单、工单等业务数据在第二阶段通过项目业务 API 实时查询，不进入通用知识索引或长期 Memory。
...
8. PostgreSQL/对象存储保存权威数据，检索和图索引均为可重建投影。
...
| 元数据 | PostgreSQL 权威 | 向量库承载全部元数据 | 事务、审计、权限和发布状态需要关系型数据库 |
| 首期检索存储 | 阿里云 Elasticsearch 向量增强版 8.17 | 同时部署 Elasticsearch + Milvus + Weaviate | 一期统一 BM25、向量、过滤和高亮，降低运维复杂度 |
...
| 异步批处理 | PostgreSQL Outbox + Kafka 负责可靠领域事件；Python Celery 负责任务执行 | 直接从 Java 写入 Celery Redis 私有消息，或将 Celery 作为业务状态机 | Kafka 可重放且与领域事件一致；Celery 适合 Python 批量并发、重试和任务编排 |
...
- 内部需求、设计、工单等资料，但必须先进入 Source Zone，再进入 Governance Zone，经脱敏、改写和审批后发布。

第一阶段不得进入对外 Serving Index：

- 用户订单、账户、余额、权益和客户级业务数据。
- 客户 PII、未审核客服对话和工单原文。
- 源代码、密钥、凭证和内部网络信息。
- 可见范围不明确的草稿。
- 消费者聊天记录和上传内容。
...
- Analyzer Profile、术语/同义词字典、Chunker、Embedding Space、RRF、Reranker 和 Index Schema Version。
...
SourceRevision / Tombstone
  ...
  -> Alias / Deployment Revision 原子切换
```
...
- Celery 的 chain/group/chord 只负责任务执行、并发和重试；任务状态、Release 水位、激活和回滚真相仍在 PostgreSQL 与 Control Plane。
- 更新、删除和权限收窄都生成新 Revision；删除使用 Tombstone 清理 BM25、向量、缓存和后续图索引。
- PostgreSQL 保存任务状态和发布水位，Elasticsearch 只保存可重建的 Serving Projection。
- “实时更新”定义为内容通过必要审批后，P95 五分钟内完成增量构建和原子激活。
- 全量重建与在线增量使用独立队列和资源池，不能互相阻塞。

### 6.5 Deployment 激活、回滚与 Session 换版

普通发布通过新 Deployment Revision 原子激活，回滚通过重新指向上一已验证 Revision 完成。Release 激活不改变正在执行的消息上下文，也不会在回答生成中途换版：
...
活跃 Session 和短期 Memory 保存于 Redis；登录用户明确保存的 Conversation 加密写入 PostgreSQL Conversation Store。匿名会话默认 24 小时。Conversation 不作为知识索引数据源。
...
第一阶段使用阿里云 Elasticsearch 向量增强版 8.17 统一承载 BM25、向量、正文、过滤和高亮，阿里云 OpenSearch 向量检索版只作为同条件挑战者。Project、Release、Locale、Access Segment、有效期、撤回状态、Embedding Space 和 Configuration Filter 必须在 BM25 与 Vector 各自取 Top-K 前生效，不能先裸搜再过滤。

RRF 由受约束 Domain Kernel 对两路独立 Rank 执行，按 `chunk_id` 去重并保留原始 Rank/Score；不使用“在 Vector 候选内叠加 BM25 分数”或搜索产品私有混合分数替代统一融合合同。首期默认 `BM25 Top 50 + Vector Top 50 -> RRF(k=60) Top 50 -> Reranker Top 10`，参数调整必须使用同一密封评测集重新验证。
...
- Python `assistant-runtime` 是 Kernel 的唯一实现位置；它不直接修改 PostgreSQL 中的业务、权限、Session 或 Release 权威状态。
...
- Search 和 Answer 向客户端返回实际 Project、Project Version、Project Execution Binding、Knowledge Release、Citation 和知识生效时间；Assistant/Prompt Version、Index Watermark、逻辑模型和物理模型版本写入内部审计与 Trace。
...
第一阶段不接受浏览器直接传入订单、客户号等敏感业务上下文。第二阶段此类上下文必须由宿主后端签名并执行对象级授权。
...
| 存储 | 职责 | 数据性质 |
| --- | --- | --- |
| PostgreSQL | 项目、账号映射、Grant、文档修订、Release、Assistant、Deployment 和任务状态 | 权威元数据 |
| 对象存储 | 原始资料、治理内容、不可变 Revision 和发布资产 | 权威内容 |
| Elasticsearch | Published BM25、向量、正文、过滤和高亮 | 可重建 Serving Projection |
| PostgreSQL Conversation Store | 登录用户 Conversation、Thread 元数据、收藏和反馈 | 加密持久数据 |
...
- Source/Governance Index 与 Published Serving Index 完全分离，使用独立凭证和网络策略。
- 即使共享 Elasticsearch Cluster，也不能共享无项目、Release 和 Access Policy 边界的 Collection。
- Staging 与 Active Serving Projection 按 Release 隔离，活动 Alias 只指向已验证版本；激活、回滚和撤回语义以 6.5 节为准。
- 增量索引只重算受影响文档；具体采用 Delta Segment、分区索引或重建策略，由容量测试和 Elasticsearch 专项设计确定。
- PostgreSQL 保存 Release、任务状态和 Index Watermark，Elasticsearch 不作为发布状态真相源。
...
- PostgreSQL、Kafka、Elasticsearch 和对象存储优先采用云托管服务。
...
- 会话、审计、Source Revision 和 Release 的保留期由合规策略确定，并支持跨 PostgreSQL、对象存储、Elasticsearch 和 Redis 的删除传播。
...
- PostgreSQL 多可用区高可用、PITR，初始目标 `RPO <= 5 分钟`、`RTO <= 2 小时`。
- 对象存储启用版本控制和生命周期策略。
- Conversation Store 配置加密备份和恢复演练，RPO/RTO 在容量与合规基线确定后冻结。
- Elasticsearch 必须能从 Release Manifest、对象存储和 Kafka 事件重建；重建 RTO 通过容量测试确定。
- Redis 不作为持久权威，丢失后重新登录或重建缓存与短期 Memory。
- 每季度执行备份恢复、Grant 撤销、知识撤回、模型故障和 Elasticsearch 重建演练。
...
- PostgreSQL、对象存储、Elasticsearch、Redis、Kafka 和基础 K8s 部署。
...
| 候选能力 | 引入条件 | 架构约束 |
| --- | --- | --- |
| Milvus | Elasticsearch 经分片、HNSW/量化、冷热分层和扩容后仍不满足向量 P95、积压或成本目标 | 只接管 Vector Recall；PostgreSQL 仍是元数据权威，Elasticsearch 保留 BM25 |
...
跨项目 GraphRAG 分别查询各项目的已发布图，再聚合经过权限校验的结果。搜索引擎迁移先使用 Release 事件回放构建影子索引，执行离线评测、双读和项目级小流量切换，不把永久在线双写作为一致性真相源。
