...
第一阶段前端采用 Vue 3 + TypeScript，平台业务后端采用 Java/Spring Boot，模型、检索、文档处理和批任务采用 Python。若 Java 与 Python 分别持有 Session、Release、Scope、Evidence 或 Citation 的部分规则，会形成两个领域控制面，无法可靠证明发布、撤回、引用和输出门禁始终一致。

Kafka 已承担可重放领域事件，Redis 用于在线临时状态。Python Celery 适合文档解析、切分、Embedding、批量索引和评测，但不能成为业务状态机或可靠事件日志。

## 2. 决策

1. 采用 Monorepo，包含 Vue 前端、Java `platform-api`、Python `assistant-runtime`、Python `batch-worker` 与共享 OpenAPI/SSE/Kafka Schema。
...
7. Python 依赖和工作区统一使用 `uv` 与单一 `uv.lock`。第一阶段不建设 React Adapter。
...
| --- | --- | --- |
| `portal-web` | Vue 3 + TypeScript | Web 请求和静态资源负载 |
...
- 出现有明确需求和维护者的 React 宿主项目；届时以 Web Component 公开合同为基础单独评审 Adapter。
